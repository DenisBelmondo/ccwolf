...
void DrawFloorAndCeiling(byte *vbuf, unsigned vbufPitch, int min_wallheight)
{
    ...
    for (int y = y0, bot_offset = bot_offset0, top_offset = top_offset0; y < halfheight;
         y++, bot_offset += vbufPitch, top_offset -= vbufPitch)
    {
...
#endif
        for (int x = 0, bot_add = bot_offset, top_add = top_offset; x < viewwidth; x++, bot_add++, top_add++)
        {
            if (wallheight[x] >> 3 <= y)
            {
