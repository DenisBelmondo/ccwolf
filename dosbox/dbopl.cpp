...
template <Operator::State yes> Bits Operator::TemplateVolume()
{
    Bit32s vol = volume;
    Bit32s change;
    switch (yes)
...
template <bool opl3Mode> INLINE void Channel::GeneratePercussion(Chip *chip, Bit32s *output)
{
    Channel *chan = this;
...
void Chip::WriteReg(Bit32u reg, Bit8u val)
{
    ...
    case 0x00 >> 4:
        ...
        {
            // MAME says the real opl3 doesn't reset anything on opl3 disable/enable till the next write in another
            // register
            if (!((opl3Active ^ val) & 1))
                return;
